...
public class RecordLineInTest extends BaseTestCase {
	...
	public RecordLineInTest(Context context, Handler handler,
	...
	private void startTest() {
		...
		view = LayoutInflater.from(mContext)
				.inflate(R.layout.test_record, null);
	...
	View.OnClickListener mOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			if (v == btnRecord) {
				stopPlay();
				startRecord();
				new Thread(new SwitchChannelThread()).start();
			} else if (v == btnPlay) {
				stopRecord();
				startPlay();
	...
	private static final int MSG_CHANNEL_LINEIN = 1;
	...
	public Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_STOP_RECORD:
				...
				Toast.makeText(
						mContext,
						"The record has enough " + recordTimeMax / 1000
								+ "s, stop recording automatically!",
						Toast.LENGTH_LONG).show();
				break;

			case MSG_CHANNEL_LINEIN:
				SettingUtil.SaveFileToNode(new File("/proc/in_sel"), "1");
				break;
	...
	public void startRecord() {
		...
		try {
			...
			Toast.makeText(mContext, "Record Failed!", Toast.LENGTH_LONG)
					.show();
			Log.e(TAG, "Record Failed!");
		}
		// mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_CHANNEL_LINEIN),1000);
		mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_STOP_RECORD),
				recordTimeMax);
	}

	boolean isTesting = false;

	class SwitchChannelThread implements Runnable {
	...
	public void startPlay() {
		...
		try {
			...
			Toast.makeText(mContext, "Play Record Failed!", Toast.LENGTH_LONG)
					.show();
	...
	public boolean onTesting() {
		...
		recordTimeMax = StringUtils.parseInt(
				attachParams.get(ParamConstants.RECORD_TIME_MAX),
				RECORD_MAX_TIME);
		LOGV("recordTimeMax" + recordTimeMax);
		startTest();
		AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
		builder.setTitle(R.string.rec_linein_title);
