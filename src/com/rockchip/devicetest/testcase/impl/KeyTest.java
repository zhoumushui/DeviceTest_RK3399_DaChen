...
public class KeyTest extends BaseTestCase {
	...
	public void onTestInit() {
		...
		//mButtonKeyMaps.put(KeyEvent.KEYCODE_F12, R.id.btn_recovery);
		//mButtonKeyMaps.put(KeyEvent.KEYCODE_VOLUME_MUTE, R.id.btn_recovery);
		mButtonKeyMaps.put(KeyEvent.KEYCODE_VOLUME_UP, R.id.btn_recovery);
		
		mButtonKeyMaps.put(KeyEvent.KEYCODE_POWER,  R.id.btn_power);
	...
	public boolean onTesting() {
		...
		filter.addAction(BRAODCAST_MSG_SCREE_OFF);
		filter.addAction(BRAODCAST_MSG_SCREE_ON);
		mContext.registerReceiver(mBraodcasetReceiver, filter); 
		...
		builder.setPositiveButton(mContext.getString(R.string.pub_success), new OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				mContext.unregisterReceiver(mBraodcasetReceiver); 
				onTestSuccess();
			}
		});
		...
		mDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
			public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
				...
				Log.d(TAG, "keyCode:"+keyCode+" actionCode:"+actionCode);
				int btnID = 0;
	...
	private static final String BRAODCAST_MSG_SCREE_OFF = "android.intent.action.SCREEN_OFF";
	private static final String BRAODCAST_MSG_SCREE_ON = "android.intent.action.SCREEN_ON";
	private BroadcastReceiver mBraodcasetReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			final String action = intent.getAction();
			Log.d(TAG, "action:"+action);
			if (BRAODCAST_MSG_SCREE_OFF.equals(action)) {
				setButtonBackgroundDown(R.id.btn_power);				
			} else if (BRAODCAST_MSG_SCREE_ON.equals(action)) {
				setButtonBackgroundUp(R.id.btn_power);
			}
		};
