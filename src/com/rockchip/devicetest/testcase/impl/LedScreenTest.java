...
public class LedScreenTest extends BaseTestCase {
	...
	public LedScreenTest(Context context, Handler handler, TestCaseInfo testcase) {
		super(context, handler, testcase);
		mLEDHandler = new LEDHandler(handler.getLooper());
	}

	public boolean onTesting() {
		SettingUtil.setLedScreenMode(3);
		
		AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
		builder.setTitle(R.string.led_screen_title);
		builder.setMessage(R.string.led_screen_msg);
		/*
		 * builder.setSingleChoiceItems(R.array.test_led, 0, new
		 * DialogInterface.OnClickListener(){
		 * 
		 * @Override public void onClick(DialogInterface dialog, int which) {
		 * if(which==0){ mStartOnOff = false; LEDSettings.onLed(); mLEDMode =
		 * LEDMode.ON; }else if(which==1){ mStartOnOff = false;
		 * LEDSettings.offLed(); mLEDMode = LEDMode.OFF; }else if(which==2){
		 * mStartOnOff = true; mLEDHandler.sendEmptyMessageDelayed(MSG_ON_OFF,
		 * 100); } } });
		 */
		builder.setPositiveButton(mContext.getString(R.string.pub_success),
				new OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						onTestSuccess();
						SettingUtil.setLedScreenMode(4);
					}
				});
